#Couchbase-lite, Dagger v1 and Robolectric Integration#
...
Because Couchbase lite's Database manager is *Final* and requires native code, its not easy mock or to integrate with Robolectric. Therefore, in order to allow Java VM testing with Robolectric it is necessary to use custom interfaces and a dependency injection framework that will allow the injection of test classes when testing. To acheive this, Dagger 1.2.2 has been used in this example.
...
3. `PersistanceAdapter.java` is a custom interface that defines supports for INSERT and GET operations on `Map` objects. This interface is required later when mocking & testing.
...
At runtime, Dagger, the `AbstractBaseActivity` class and the `App` application class take care of constructing an `ObjectGraph` and inserting the required `@Module` classes so that `@Inject` requirements can be met. The integration tests in the App project illustrate this integration is working as expected.

###The Robolectric Tests###
In the `app-test` codebase you'll see the following...

1. `MyTestActivity.java` extends the MyActivity class and @overrides the `getModules()` method. This method returns a `MyTestActivityModule` (explained next).
2. `MyTestActivityModule.java` defines a Dagger @Module that can construct a `PersistanceManager` class for use when injecting the `MyTestActivity` with it's dependency. It will @Provide a fake, programmable 'PersistenceManager' mock, not a real perirtance manager as is expected.
3. `MyActivityRobolectricTest.java` is a normal Robolectric test, but it's controller builds a new `MyTestActivity`. The `testClickingSaveButtonSavesMapAndDisplaysId()` method tests the _Save_ button has the required efect by pre-programming the PersistenceManager mock and verifying that it's been called.
